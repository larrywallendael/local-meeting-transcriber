...
export function Sidebar({ view, selectedFolder, onNavigate, isOpen, onToggle, onSearchOpen, onSettingsOpen, onContactOpen }: SidebarProps) {
  ...
  const { history, trashedJobs, restoreFromTrash, deletePermanently, folders } = useJobs();
  ...
  return (
    <aside className="flex h-full flex-col border-r border-border/70 px-3 py-5">
      ...
      <nav className="flex-1 space-y-6">
        <button
          className={`flex w-full items-center rounded-md py-2 text-[15px] ${
        ...
        {isOpen && (
          ...
          {foldersOpen && (
            <div className="mt-2 space-y-1">
              {folders.length === 0 && (
                <p className="px-2.5 py-1 text-[13px] text-muted-foreground">
                  No folders yet.
                </p>
              )}
              {folders.map((folder) => {
          ...
          {isTrashOpen && trashPosition && createPortal(
            <>
              ...
              <div className="flex items-center justify-between pb-3 text-[16px] font-medium">
                <span>Trash</span>
                <button
                  className="close-button text-muted-foreground hover:text-foreground"
                  onClick={() => setIsTrashOpen(false)}
                  type="button"
                >
                  ×
